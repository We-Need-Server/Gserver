...
func (g *Game) GetGameDeltaState() map[uint32]player.PlayerPosition {
	...
	for qPort, p := range g.Players {
		gameDeltaState[qPort] = player.NewPlayerPosition(p.XDelta, p.YDelta, p.ZDelta, p.YawDelta, p.PTDelta)
...
func (g *Game) GetGameState() map[uint32]player.PlayerPosition {
	...
	for qPort, p := range g.Players {
		gameState[qPort] = player.NewPlayerPosition(p.PositionX, p.PositionY, p.PositionZ, p.YawAngle, p.PTAngle)
	}
