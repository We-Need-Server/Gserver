...
func (n *Network) ReadyTcp(blueTeamDb map[uint32]*user.User, redTeamDb map[uint32]*user.User, loginFunc func(uint32, *net.TCPAddr) (uint32, error)) (*receiver.TcpReceiver, *sender.TcpSender) {
	...
	n.tcpListener = tcpLn
	n.tcpReceiver = receiver.NewTcpReceiver(tcpLn, loginFunc, n.communicateTcpSender)
	n.tcpSender = sender.NewTcpSender(n.listenUdpAddr, blueTeamDb, redTeamDb)
